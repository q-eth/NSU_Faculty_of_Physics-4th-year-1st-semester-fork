...
\begin{tikzpicture}[>=Latex,every node/.style={text=rtext},x=2.45cm,y=2.45cm]
    \draw[ovfaxis] (-0.35,0) -- (2.15,0)
        node[below left=1pt,text=rtext] {$x$};
    \draw[ovfaxis] (0,-0.55) -- (0,1.45)
        node[left=1pt,text=rtext] {$y$};
    ...
    \node[text=rtext!85,anchor=south west]
        at (1.15,1.15) {$y=x$};
    \node[text=rc4,anchor=south west]
        at (1.25,0.28) {$y=\cos x$};
    \node[text=rtext!80,anchor=north west]
        at (0.78,0.70) {$x^*$};
\end{tikzpicture}
...
\begin{tikzpicture}[>=Latex,every node/.style={text=rtext},x=0.75cm,y=0.75cm]
    \clip (-5.3,-4.2) rectangle (5.3,4.2);

    \draw[ovfaxis] (-5.15,0) -- (5.15,0)
        node[below,text=rtext] {$x$};
    \draw[ovfaxis] (0,-4.0) -- (0,4.0)
        node[left,text=rtext] {$y$};
...
\end{tikzpicture}
    ...
    \label{fig:ovf_tan_fixed_points}
\end{rbox}
...
\begin{tikzpicture}[>=Latex,every node/.style={text=rtext},x=1.5cm,y=1.0cm]
    ...
    \node[text=rtext,below] at (0.70,0) {$a$};
    \node[text=rtext,below] at (2.40,0) {$b$};
    \node[text=rtext!85,above] at (1.5,0) {$x_1^*$};
    \node[text=rtext!85,above] at (4.5,0) {$x_2^*$};
...
\begin{tikzpicture}[>=Latex,every node/.style={text=rtext},x=2.0cm,y=1.15cm]
    ...
    \node[text=rtext,below] at (\aa,0) {$a_0$};
    \node[text=rtext,below] at (\bb,0) {$b_0$};
    \node[text=rtext!85,below=3pt] at (\xxone,0) {$x_1$};
    \node[text=rc3,above=3pt] at ({sqrt(2)},0) {$x^*$};
...
\end{tikzpicture}
    ...
    \label{fig:ovf_bisection_first_step}
\end{rbox}
...
\begin{tikzpicture}[>=Latex,every node/.style={text=rtext},x=3.0cm,y=3.0cm]
    ...
    \node[below=3pt] at (\xzero,0) {$x_0$};
    \node[below=3pt] at (\xone,0) {$x_1$};
    \node[above=3pt] at (\xtwo,0) {$x_2$};
...
\end{tikzpicture}
    ...
    \label{fig:ovf_simple_iterations}
\end{rbox}
...
\centering
\begin{tikzpicture}[>=Latex,every node/.style={text=rtext},x=2.1cm,y=2.1cm]
    ...
    \node[text=rtext!85,anchor=south west] at (1.35,1.35) {$y=x$};
    \node[text=rc4,anchor=north west] at (1.28,1.15) {$y=\varphi(x)$};
    \node[text=rtext,below] at (1,0) {$x^*$};
...
\end{minipage}
    ...
    \captionof{figure}{Сходящийся и расходящийся итерационные процессы.}
    \label{fig:ovf_iteration_convergence}
\end{rbox}
...
\begin{rbox}[left=18pt,right=18pt,top=14pt,bottom=10pt]{[]}
    ...
    \begin{tikzpicture}[
        ...
        % Постоянный корректирующий множитель:
        % x_{k+1}=x_k-\lambda f(x_k). Поэтому все наклонные
        % отрезки имеют один и тот же наклон 1/\lambda.
        \pgfmathsetmacro{\lam}{0.20}

        \pgfmathsetmacro{\xzero}{3.00}
        \pgfmathsetmacro{\fzero}{ovff(\xzero)}
        \pgfmathsetmacro{\xone}{\xzero-\lam*\fzero}
        \pgfmathsetmacro{\fone}{ovff(\xone)}
        \pgfmathsetmacro{\xtwo}{\xone-\lam*\fone}
        \pgfmathsetmacro{\ftwo}{ovff(\xtwo)}
        \pgfmathsetmacro{\xthree}{\xtwo-\lam*\ftwo}
        \pgfmathsetmacro{\fthree}{ovff(\xthree)}

        \draw[ovfaxis] (-0.10,0) -- (3.65,0)
            node[below left=1pt] {$x$};
        \draw[ovfaxis] (0,-1.25) -- (0,3.55)
            node[left=1pt] {$y$};
        ...
        \draw[ovfguide] (\xthree,0) -- (\xthree,\fthree);

        % Несколько последовательных итераций с одним и тем же lambda.
        \draw[ovftangent,-{Latex[length=1.8mm]}]
            (\xzero,\fzero) -- (\xone,0);
        \draw[ovftangent,-{Latex[length=1.8mm]}]
            (\xone,\fone) -- (\xtwo,0);
        \draw[ovftangent,-{Latex[length=1.8mm]}]
        ...
        \fill[rc3] (1.2,0) circle (1.7pt);

        % При выбранном lambda точки достаточно разнесены,
        % поэтому подписи можно расположить непосредственно у оси.
        \node[font=\small,below=4pt] at (\xzero,0) {$x_0$};
        \node[font=\small,below=4pt] at (\xone,0) {$x_1$};
        \node[font=\small,above=9pt,xshift=-7pt] at (\xtwo,0) {$x_2$};
        \node[font=\small,below=4pt] at (\xthree,0) {$x_3$};
        \node[font=\small,text=rc3,above=4pt] at (1.2,0) {$x^*$};

        \node[text=rc4,anchor=south west] at (2.55,1.76) {$y=f(x)$};

        \draw[rtext!65,line width=0.7pt]
            (\xone+0.23,0)
            arc[start angle=0,end angle=78.7,radius=0.23];
        \node[text=rtext!75,anchor=west]
            at (\xone+0.32,0.47) {$\alpha$};
    \end{tikzpicture}
    ...
    \captionof{figure}{
        Метод простых итераций с корректирующим множителем:
        несколько последовательных шагов при постоянном $\lambda$.
        Все наклонные отрезки имеют одинаковый наклон
        $\tan\alpha=1/\lambda$.
    }
    \label{fig:ovf_corrected_iterations}
\end{rbox}
...
\begin{tikzpicture}[>=Latex,every node/.style={text=rtext},x=1.75cm,y=0.95cm]
    ...
    \node[below=4pt] at (\xzero,0) {$x_0$};
    \node[below=4pt] at (\xone,0) {$x_1$};
    \node[below=4pt] at (\xtwo,0) {$x_2$};
    \node[text=rc3,above left=2pt] at (1.2,0) {$x^*$};
